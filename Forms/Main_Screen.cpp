...
//	OpenGL_Panel_1.Simulation_System = Simulation_System;
	...
	Timestep_Edit->Text = Simulation_System.Two_Dim_System.dt;

	Main_Screen_Form->Caption = "4D SIMULATOR. MODEL: " + Simulation_System.Model_Name;
}
//---------------------------------------------------------------------------
...
void __fastcall TMain_Screen_Form::Start_Sim_ButtonClick(TObject *Sender)
{
	...
	double prev_time_s=0,prev_time_ecg=0,prev_time_ectopy=0;
	...
	prev_time_ectopy = Simulation_System.get_current_time();

	Simulation_System.Signal_Sampling_Timestep = Sampling_Edit->Text.ToDouble();

	int X,Y,Z,T;
	Simulation_System.get_system_size(&X,&Y,&Z,&T);
	...
	while( Simulation_Flag )
	{
		...
		// fire ectopis source
		//-----------------------------------------------------
		if( Ectopy_In_Middle_CheckBox->Checked == cbChecked)
		if( Simulation_System.get_current_time() > prev_time_ectopy +
			Ectopy_CL_ms_Edit->Text.ToDouble() )
		{
			for(int SX=-5;SX<5;SX++)
			for(int SY=-5;SY<5;SY++)
			Simulation_System.stimulate_node(0.5*X+SX,0.5*Y+SY,0,0);
...
//	long History_Size = Simulation_System.get_history_vector_size();
	...
	for(int s2=0;s2<HD_GRID_SIZE;s2++)
	{
		...
		for(long t=2;t<Simulation_System.VOLTAGE_HISTORY[(int)cx][(int)cy].DVector.size();t++)
		{
...
void __fastcall TMain_Screen_Form::Generate_Coupling_v1_ButtonClick(TObject *Sender)
{
	...
	{
	Simulation_System.Lines_No_v1 = Lines_No_Edit->Text.ToDouble();
	Simulation_System.Line_Perc_Leng_v1 = Perc_Len_Edit->Text.ToDouble();
	Simulation_System.Line_Perc_D = PL_Edit->Text.ToDouble();
	Simulation_System.add_coupling(2,false);
	...
	{
	Simulation_System.Diff_v1_Number = NDiffv1_Edit->Text.ToDouble();
	Simulation_System.Diff_v1_Radius = DRadius_Edit->Text.ToDouble();
	Simulation_System.Ring_v1_Width = AR_Radius_Edit->Text.ToDouble();
	Simulation_System.Ring_v1_PercD = AR_Perc_Edit->Text.ToDouble();
	Simulation_System.add_coupling(1,true);
	}
	else
	{
	Simulation_System.Diff_v1_Number = NDiffv1_Edit->Text.ToDouble();
	Simulation_System.Diff_v1_Radius = DRadius_Edit->Text.ToDouble();
	Simulation_System.add_coupling(1,false);
	}
	...
	{
	Simulation_System.Disk_Radius = CD_R_Edit->Text.ToDouble();
	Simulation_System.add_coupling(3,false);
	}
...
void __fastcall TMain_Screen_Form::SET_MI_ButtonClick(TObject *Sender)
{
	...
	Simulation_System.set_mi_distribution(
...
/*
	...
	double Mean_Coherence = 0;
		...
		if( Simulation_System.VOLTAGE_HISTORY[i][j].DVector.size() > 0 &&
			Simulation_System.VOLTAGE_HISTORY[i+Radius][j].DVector.size() > 0 &&
			Simulation_System.VOLTAGE_HISTORY[i-Radius][j].DVector.size() > 0 &&
			Simulation_System.VOLTAGE_HISTORY[i][j-Radius].DVector.size() > 0 &&
			Simulation_System.VOLTAGE_HISTORY[i][j+Radius].DVector.size() > 0 )
		{
		Mean_Coherence += NumAlg.get_MPC_between_electrograms(
			&Simulation_System.VOLTAGE_HISTORY[i][j].DVector,
			&Simulation_System.VOLTAGE_HISTORY[i-Radius][j].DVector,
			BCL,El_Type,Delay);
		Mean_Coherence += NumAlg.get_MPC_between_electrograms(
			&Simulation_System.VOLTAGE_HISTORY[i][j].DVector,
			&Simulation_System.VOLTAGE_HISTORY[i+Radius][j].DVector,
		...
		Mean_Coherence += NumAlg.get_MPC_between_electrograms(
			&Simulation_System.VOLTAGE_HISTORY[i][j].DVector,
			&Simulation_System.VOLTAGE_HISTORY[i][j-Radius].DVector,
			BCL,El_Type,Delay);
		Mean_Coherence += NumAlg.get_MPC_between_electrograms(
			&Simulation_System.VOLTAGE_HISTORY[i][j].DVector,
			&Simulation_System.VOLTAGE_HISTORY[i][j+Radius].DVector,
...

{
	...
	for(int j=Grid_Sampling;j<Simulation_System.Two_Dim_System.Size_Y;j+=Grid_Sampling)
	{
		...
		NumAlg.stdev_cor_vec_long(&Simulation_System.ISIs,&ISI_mean,&ISI_SD);
...
//	ShowMessage("Remember to point the origin point (right mouse click + push as...) ");
	...
	SenderS = Simulation_System.Activations_Positions;
	...
	Simulation_System.compute_ISIs_XY(i,j);
	Receiver = Simulation_System.Activations_Positions;
...
void __fastcall TMain_Screen_Form::Generatesimulationfile1Click(TObject *Sender)
{
	...
	while( Simulation_System.get_current_time() < End_Time )
	{
			...
			Simulation_System.Signal_Sampling_Timestep )
		{
			...
			{
				df << (double)((int)(Simulation_System.VOLTAGE_HISTORY[x][y].DVector[0]*100))/100 << ",";
				Simulation_System.VOLTAGE_HISTORY[x][y].DVector.clear();
			}
...
void TMain_Screen_Form::paint_slab()     // gogh
{
...
{
	ShowMessage("Two parameters are used: 'basic CL' and 'CV mesh spacing'");
	Simulation_System.calculate_velocity_field(
		Basic_CL_Edit->Text.ToDouble(),

		CV_Mesh_Spacing_Edit->Text.ToInt()  );
...
{
Recalculate_CL_ButtonClick(this); // im not sure why it has to be executed before (no time to investigate)

	MPC_Dist_Chart->Series[0]->Clear();
	MPC_Dist_Chart->Series[1]->Clear();

	Ask_For_Single_Item_Form1->Caption = "Intervals number";
	Ask_For_Single_Item_Form1->Edit_Box->Text = "50";
	Ask_For_Single_Item_Form1->ShowModal();
	double Intervals_Number = Ask_For_Single_Item_Form1->Edit_Box->Text.ToDouble();
	...
	if( SaveDialog1->Execute() )
	{
		ofstream df(SaveDialog1->FileName.c_str() );
		for(long t=0;t<means.size();t++)
		df << t*Interval << "," << means[t] << endl;
		df.close();
	}
...
void __fastcall TMain_Screen_Form::Clear_Sig_History_ButtonClick(TObject *Sender)
{
	Simulation_System.clear_signals_history();
	ShowMessage("Signals history erased.");
}
//---------------------------------------------------------------------------

void __fastcall TMain_Screen_Form::SaveXYpointstocsvfile1Click(TObject *Sender)
{
    // 1. Check if the chart has at least one series
    if (MPC_Dist_Chart->SeriesCount() == 0)
    {
        ShowMessage("The chart does not contain any data series.");
        return;
    }

    // 2. Get the first series (assuming it's the XY scatter series)
    //    It's good practice to cast if you know the type, but accessing
    //    XValues and YValues is common for many series types.
    TChartSeries *Series = MPC_Dist_Chart->SeriesList->Items[0];
    // Example cast (optional): TPointSeries *PointSeries = dynamic_cast<TPointSeries*>(Series);

    // 3. Check if the series actually contains data points
    if (Series->Count() == 0)
    {
        ShowMessage("The selected series (Series 0) has no data points to save.");
        return;
    }

    // 4. Configure and show the Save Dialog
    SaveDialog1->Title = "Save Chart Data As";
    SaveDialog1->DefaultExt = "csv";
    // Filter for CSV files first, then all files
    SaveDialog1->Filter = "CSV Files (*.csv)|*.csv|All Files (*.*)|*.*";
    SaveDialog1->FileName = "chart_data.csv"; // Suggest a default filename

    if (SaveDialog1->Execute()) // Show the dialog and check if the user clicked "Save"
    {
        // 5. User clicked "Save", get the chosen filename
        UnicodeString FileName = SaveDialog1->FileName;

        // 6. Prepare data using TStringList (easier for text files)
        TStringList *DataLines = new TStringList();
        try
        {
            // Add a header row (optional)
            DataLines->Add("X,Y"); // Simple CSV header

            // Loop through all points in the series
            for (int i = 0; i < Series->Count(); ++i)
            {
                // Get X and Y values for the current point
                // Using ->Value[] is the standard way for TeeChart value lists
                double xValue = Series->XValues->Value[i];
                double yValue = Series->YValues->Value[i];

                // Format the X and Y values as a comma-separated string
                // Use Format or simple concatenation depending on desired precision/locale
                // UnicodeString Line = FloatToStr(xValue) + "," + FloatToStr(yValue);
                // Using Format for potentially better control (e.g., decimal places)
                // Adjust the format specifier ('%f', '%.4f', etc.) as needed
                 UnicodeString Line = System::Sysutils::Format("%.6f,%.6f", ARRAYOFCONST((xValue, yValue)));
            ...
            // 7. Save the TStringList content to the selected file
            DataLines->SaveToFile(FileName); // Handles encoding automatically (usually system default)

			// 8. Clean up: Free the memory allocated for TStringList
			delete DataLines;

			ShowMessage("Data successfully saved to " + FileName);
		}
		catch (const Exception &e)
		{
			// Handle potential exceptions during file writing or data access
			ShowMessage("Error saving file: " + e.Message);
		}
    }
    // else: User clicked "Cancel" in the Save Dialog, do nothing.
}
//---------------------------------------------------------------------------
