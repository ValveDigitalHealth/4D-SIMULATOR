...
//	Diffusion_Coefficient = 0.0001;
	...
	54.4; // jest bardziej zlozone wyrazenie na to

	Gk = 0.282000* pow((K_o/5.40000), 1.0 / 2);
	Ek = (R*T/F)*log((K_o+PNa_K*Na_o)/(K_i+PNa_K*Na_i));        // mv

	Gsi = 0.045; //stable: 0.025;  //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! changed for breakup
...
double Luo_Rudy_91_cell::get_current()
{
	return Ib + INa + Isi + IK + IK1 + IKp;
}
