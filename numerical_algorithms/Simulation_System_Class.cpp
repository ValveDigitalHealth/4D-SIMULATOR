...
int Simulation_System_Class::get_color_code(int Source, int X,int Y,int Z,int T, long Time, int Grid_Spacing,double Cycle_Length_ms)
{
	...
	if( Source == 0 )
	ptr = 255*(Two_Dim_System.V1[X][Y]-Two_Dim_System.Min_Voltage)/(Two_Dim_System.Max_Voltage-Two_Dim_System.Min_Voltage);

//	if( Source == 0 && Two_Dim_System.DX[X][Y] <= Two_Dim_System.Min_Voltage && Two_Dim_System.DY[X][Y] <= 0.1 )
//	ptr = -1;
